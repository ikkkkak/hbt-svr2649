...
func main() {
	fmt.Println("🚀 Starting MINIMAL apartments-clone-server...")
	fmt.Println("🔍 Debug: Main function started")

	// Check environment
	if os.Getenv("RENDER") == "" {
		fmt.Println("🔍 Debug: Running locally")
	} else {
		fmt.Println("🌐 Running on Render (production)")
	}

	fmt.Println("🔧 Creating minimal Iris app...")
	app := iris.New()

	// Health check endpoint - CRITICAL for Render
	fmt.Println("🔧 Setting up health check endpoint...")
	app.Get("/health", func(ctx iris.Context) {
		ctx.JSON(iris.Map{"status": "ok", "message": "Minimal server is running"})
	})
	...
	fmt.Printf("🚀 Minimal server starting on %s\n", addr)
	...
	fmt.Println("🎯 Attempting to start minimal server with router.Run()...")
	if err := app.Run(iris.Addr(addr)); err != nil {
		log.Fatalf("❌ Minimal server failed to start: %v", err)
	}
}
